...
test('frontend import workspace honors navigation route money and permission contracts', function () {
    ...
    expect($createPage)
        ...
        ->toContain('Pendiente de análisis')
        ->not->toContain('Revisión no disponible')
    ...
    expect($previewPage)
        ...
        ->toContain('OwnRevenueImportAnalysisController')
        ->toContain('return_to_preview')
        ->toContain('Volver a analizar')
    ...
    expect($slot)
        ...
        ->toContain('@/routes/finance/own-revenue/budgets/imports/files')
        ->toMatch('/<Link\s+href=\{\s*preview\(\{[\s\S]*?\}\)\.url\s*\}\s*>\s*Ver vista previa\s*<\/Link>/')
        ->not->toContain('Parser pendiente')
